...
std_error1 = std_errors1(2);
%cc_matrix = corrcoef(time(1:15),Log_cpm(1:15));
%R_squared = (cc_matrix(1,2))^2;
%std_error1 = std(time(1:15))*sqrt(1-R_squared);
...
std_error2 = std_errors2(2);
%cc_matrix = corrcoef(time(16:31),Log_cpm(16:31));
%R_squared = (cc_matrix(1,2))^2;
%std_error2 = std(time(16:31))*sqrt(1-R_squared);
...
std_error3 = std_errors3(2);
%cc_matrix = corrcoef(time,log(src_cpm_corrected));
%R_squared = (cc_matrix(1,2))^2;
%std_error3 = std(time)*sqrt(1-R_squared); 
...
for i = 1:3
    SigmaHalfLife(i) = Error(i) * HalfLife(i)/lambda(i);
end
for i = 1:3
    NumberOfSigmas(i) = (HalfLife(i) - true_value)/SigmaHalfLife(i);
end

t = table(DataPoints',lambda',HalfLife',Error',NumberOfSigmas'); 
t = renamevars(t,["Var1" "Var2" "Var3" "Var4" "Var5"],["Data Points" "Lambda" "Half Life" "Error" "Number of std. devs."]);
